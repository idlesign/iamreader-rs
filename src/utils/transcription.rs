...
pub struct TranscriptionTask {
    ...
    /// Original project identity, distinct from the resolved filesystem location.
    pub project_file_path: PathBuf,
    pub previous_hint: String,
}
...
impl TranscriptionWorker {
    pub fn new(
        ...
        Self {
            ...
            model_path,
            debug,
            threads: crate::utils::inference::thread_budget(num_cpus::get()),
        }
    }

    /// Only the opt-in resource probe overrides the production thread budget.
    #[cfg(test)]
    pub fn with_profile_threads(mut self, threads: usize) -> Self {
        assert!((1..=32).contains(&threads));
    ...
    pub fn run(self) {
        ...
        loop {
            match self.task_rx.recv() {
                Ok(task) => {
                    if self.debug {
                        debug!("Received transcription task: {:?}", task.file_path);
                    }
                    info!("Starting transcription for file: {:?}", task.file_path);

                    match Self::transcribe_file(&ctx, &task.file_path, self.debug, self.threads) {
                        Ok(text) => {
                            ...
                            // Send the result to the single owner of the project. Applying it by
                            // path avoids stale indices after inserting/deleting another fragment.
                            if let Err(e) = self.update_tx.send(TranscriptionUpdate {
                                file_path: task.project_file_path,
                                previous_hint: task.previous_hint,
                                text: hint_text,
                            }) {
    ...
    fn transcribe_file(
        ...
        let samples = match spec.sample_format {
            ...
            hound::SampleFormat::Int => {
                ...
                let f32_samples: Vec<f32> =
                    all_samples.iter().map(|&s| s as f32 / 32768.0).collect();
        ...
        let text = if samples.len() <= first_samples {
            ...
            Self::run_whisper_segments(ctx, &samples, debug, threads)?
            ...
            if debug {
                debug!(
                    "Transcribing first {}s ({} samples) and last {}s ({} samples)",
                    FIRST_SECS,
                    first.len(),
                    LAST_SECS,
                    last.len()
            ...
            let t1 = Self::run_whisper_segments(ctx, first, debug, threads)?;
            let t2 = Self::run_whisper_segments(ctx, last, debug, threads)?;
            let t1 = t1.trim();
            let t2 = t2.trim();
    ...
    fn convert_to_mono_16khz(samples: &[f32], channels: u16, sample_rate: u32) -> Vec<f32> {
